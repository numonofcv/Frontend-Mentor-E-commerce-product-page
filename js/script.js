...
function updateCartEmptyState() {
  if (cartItems.children.length === 0) {
    // Use <li> instead of <p> for empty cart message for valid HTML
    cartItems.innerHTML = '<li class="cart__empty">Your cart is empty.</li>';
  } else {
      ...
      const itemHTML = `
        <li class="cart__item" data-id="${productId}">
          ...
          <div class="cart__item-details">
            ...
            <p class="cart__item-price">$${price.toFixed(
              2
            )} x <span class="cart__item-qty">${qty}</span> 
            <span class="cart__item-total">$${(price * qty).toFixed(
              2
            )}</span></p>
          </div>
